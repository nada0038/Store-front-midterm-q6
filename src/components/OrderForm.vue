...
export default {
  ...
  methods: {
    async fetchProducts() {
      try {
        const response = await fetch(`https://product-service-h8gsc5gyeaa9aqaq.westus-01.azurewebsites.net/products`);        
        if (response.ok) {
    ...
    async submitOrder() {
      ...
      try {
        const response = await fetch(`https://order-service-aga4g6a0g5bjaxew.westus-01.azurewebsites.net/orders`, {
